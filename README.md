...
| **Команды в чате** | префикс **`.`** (меняется) | `.bind`, `.toggle`, `.config` и др. Сообщение с командой **не уходит на сервер**. Пока вы набираете команду, над чатом виден список **«Доступные команды»**. |
...
4. В игре нажмите **INSERT**: откроется меню. Команды: откройте чат и наберите `.` — над чатом появится
   список **«Доступные команды»** (или `.help`).
...
Как только в чате набран префикс (`.`), над полем ввода появляется список **«Доступные команды»**; дальше он
сужается по набранному слову (`.bi` — только `.bind` и `.binds`) и исчезает, когда чат закрыт. `.help` показывает
тот же список на 20 секунд.

![Доступные команды](docs/menu-commands.png)

| Команда | Что делает |
|---|---|
| `.help` | список «Доступные команды» |
| `.status` | что работает: хук клавиатуры и мыши, курсор (в мире или нет), чат, меню и почему оно не открывается |
...
* **Меню трея**: загрузить мод, проверить обновления, выгрузить мод, «Проверить мод» (`.status` уведомлением
  Windows), «Включить меню в игре» (после автоматического выключения), открыть папку настроек, импорт/экспорт
  конфига, фон и шрифт для меню игры, запуск вместе с Windows, выход.
...
* **Меню не открывается**: если меню не может открыться, **INSERT** сам сообщает причину (уведомлением
  в игре, если оно видно, и уведомлением Windows через лаунчер): меню выключено, ещё не запущено (первые 20 секунд
  после старта игры), ждёт кадр игры или подключение к Direct3D зависло. Полная картина — `.status` или пункт
  трея «Проверить мод». В логе должно быть `Menu ready: Direct3D 11` или `Direct3D 12`; строки `Menu: ...`
  показывают каждый шаг подключения. Подключение идёт в отдельном потоке: даже если драйвер завис, бинды и команды
  работают, а через 10 секунд мод сообщит, на каком шаге всё встало.
* **Потерялась клавиша меню**: `.bind INSERT menu` или `.menu`.
* **Бинды не срабатывают**: с `RequireHiddenCursor=1` (по умолчанию) бинды работают только в мире, а не в меню игры,
  инвентаре или чате. Мир мод узнаёт по курсору: игра прячет его (в UWP — пустым курсором, Windows при этом
  считает его «показанным»; это учтено) или сообщает только относительные движения мыши. Пока игра ни разу не
  спрятала курсор, бинды работают везде. Если клавиша не сработала из-за курсора, мод один раз скажет об этом.
  Проверить: `.status` (строка «Курсор»). Отключить проверку: `.set General.RequireHiddenCursor 0`.
...
* `src/gui/`: меню на [Dear ImGui](https://github.com/ocornut/imgui).
  ...
  * `input.cpp` берёт клавиатуру и мышь из тех же перехватов `Keyboard::feed`/`MouseDevice::feed` и передаёт
    их в ImGui на поток отрисовки; `menu.cpp` — страницы меню, уведомления и список «Доступные команды» у чата; `theme.cpp` — цвета, размеры, готовые темы,
...
xvfb-run -a wine64 fake_game.exe BedrockQoL.dll            # 145 проверок, Direct3D 11
xvfb-run -a wine64 fake_game.exe BedrockQoL.dll --d3d12    # 145 проверок, Direct3D 12
cd ..
...
само, если прошлый запуск игры оборвался во время его старта, и включается обратно командой). Ещё проверяются
список «Доступные команды» у чата (появление, фильтр, `.help`, скрытие), объяснение от INSERT при выключенном меню,
`.status` и бинды с `RequireHiddenCursor=1`: «игра» ставит стрелку на экранах и пустой курсор в мире, как UWP. Виджеты тест находит по меткам через
