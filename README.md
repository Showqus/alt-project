...
```
Секции `config.ini`: `[General]` (выгрузка, проверка курсора), `[Menu]` (клавиша меню, шрифт, размер,
масштаб, фон, уведомления, анимация), `[Theme]` (цвета `#RRGGBBAA` и размеры меню; хранятся только
...
xvfb-run -a wine64 fake_game.exe BedrockQoL.dll            # 114 проверок, Direct3D 11
xvfb-run -a wine64 fake_game.exe BedrockQoL.dll --d3d12    # 114 проверок, Direct3D 12
cd ..
...
JSON-темы, `ResizeBuffers` при открытом меню и выгрузку с открытым меню. Виджеты тест находит по меткам через
